    ...
    try:
        with connectionBD() as conexion_MySQldb:
            with conexion_MySQldb.cursor(dictionary = True ) as cursor:
                ...
                sql = "INSERT TO empleados (nombre_empleados, apellidos_empleados, tipo_identidad, n_identidad, fecha_nacimiento, sexo, grupo_rh, email, telefono, profesion, salario, foto_perfil) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"

                valores = (dataForm['nombre_empleado'], dataForm['apellidos_empleados'], dataForm['tipo_identidad'],
        ...
        return f"Se produjo in eros en procesar_form_empleado: {str(e)}"
...
def sql_lista_empleadosBD():
    try:
        ...
        print(f"Error en la funcion sql_lista_empleadosBD: {e}")
        return None
...
def sql_detalles_empleadosBD(idEmpleado):
    try:
        ...
        print(f"Error en la funcion sql_detalles_empleadosBD: {e}")
        return None
            ...
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                querySQL = ("""
                    SELECT
                        ...
                        ORDER BY e.id_empleado DESC
                    """)
                cursor.execute(querySQL, (querySQL,))
                empleadosBD = cursor.fetchall()
        return empleadosBD
    except Exception as e:
        print(f"Error en la funcion empleadosReporte: {e}")
...
def generarReporteExcel():
    ...
    for registro in dataEmpleados:
        ...
        apellidos_empleados = registro['apellidos_empleado']
        ...
        hoja.append((nombre_empleado, apellidos_empleados, tipo_identidad, n_identidad, fecha_nacimiento, sexo, grupo_rh, email, telefono, profesion,
    ...
    archivoExcel = f"Reporte_ empleados_ {fecha_actual.strtime('%Y_%m_%d')}.xlsx"
...
def buscarEmpleadosUnico(id):
    try:
        ...
        print(f"Ocurrió un error en def buscarEmpleadosUnico: {e}")
        return []
...
def lista_usuariosBD():
    try:
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                querySQL = "SELET id_usuario, usuario, email, fecha_creado FROM usuarios"
                cursor.execute(querySQL)
